...
pub fn Ray3(comptime Scalar: type) type {
    ...
    // Hit is a 4D vector with same scalar type as the ray (xyz barycentrics, w is distance t)
    const Vec4P = vec.Vec4(P);
    ...
    return extern struct {
        ...
        pub inline fn triangleIntersect(
            ...
            if (backface_culling) {
                if (u < 0.0 or v < 0.0 or w < 0.0) return null; // no hit
            } else {
                if ((u < 0.0 or v < 0.0 or w < 0.0) and (u > 0.0 or v > 0.0 or w > 0.0)) return null; // no hit
            }

            var det: P = u + v + w;
            if (det == 0.0) return null; // no hit
            ...
            // Since Ray.Hit is represented by a Vec4, t is the last element of that vector
            var hit: Hit = Vec4P.init(0, 0, 0, math.inf(P));

            if (backface_culling) {
                if ((t < 0.0) or (t > hit.v[3] * det)) return null; // no hit
            } else {
                if (det < 0) {
                    t = -t;
                    det = -det;
...
test "triangleIntersect_basic_frontface_bc_hit" {
    ...
    const result: math.Ray.Hit = ray0.triangleIntersect(&a, &b, &c, true).?;

    const expected_t: f32 = 1;
    const expected_u: f32 = 0.6;
    const expected_v: f32 = 0.2;
    const expected_w: f32 = 0.2;
    try testing.expectApproxEqAbs(expected_u, result.v[0], 1e-6);
    try testing.expectApproxEqAbs(expected_v, result.v[1], 1e-6);
    try testing.expectApproxEqAbs(expected_w, result.v[2], 1e-6);
    try testing.expectApproxEqAbs(expected_t, result.v[3], 1e-6);
}
...
test "triangleIntersect_basic_backface_no_bc_hit" {
    ...
    const result: math.Ray.Hit = ray0.triangleIntersect(&a, &c, &b, false).?;
...
test "triangleIntersect_basic_backface_bc_miss" {
    ...
    // Reverse winding from previous test
    const result: ?math.Ray.Hit = ray0.triangleIntersect(&a, &c, &b, true);

    try testing.expect(result == null);
}

test "triangleIntersect_precise_frontface_bc_hit_f32" {
    const a: math.Vec3 = math.vec3(3164.91, 3559.55, 3044.54);
    const b: math.Vec3 = math.vec3(1011.92, 3113.34, 3674.56);
    const c: math.Vec3 = math.vec3(503.804, 2311.16, 2449.58);
    const ray0: math.Ray = math.Ray{
        .origin = math.vec3(293.293, 264.527, 225.465),
        .direction = math.vec3(0.439063, 0.652555, 0.617573),
    };
    ...
    try testing.expectApproxEqAbs(expected_u, result.v[0], 1e-5);
    try testing.expectApproxEqAbs(expected_v, result.v[1], 1e-5);
    try testing.expectApproxEqAbs(expected_w, result.v[2], 1e-5);
    try testing.expectApproxEqAbs(expected_t, result.v[3], 1e-2);
}

test "triangleIntersect_precise_frontface_bc_hit_f64" {
    const a: math.Vec3d = math.vec3d(2371.01, 3208.12, 1570.04);
    const b: math.Vec3d = math.vec3d(1412.2, 2978.36, 1501.33);
    const c: math.Vec3d = math.vec3d(2520.99, 3323.93, 1567.18);
    const ray0: math.Rayd = math.Rayd{
        .origin = math.vec3d(246.713, 279.646, 180.443),
        .direction = math.vec3d(0.497991, 0.782698, 0.373349),
    };

    const result: math.Rayd.Hit = ray0.triangleIntersect(&a, &b, &c, true).?;

    const expected_t: f64 = 3660.17;
    const expected_u: f64 = 0.56102;
    const expected_v: f64 = 0.33136;
    const expected_w: f64 = 0.10761;
    try testing.expectApproxEqAbs(expected_u, result.v[0], 1e-4);
    try testing.expectApproxEqAbs(expected_v, result.v[1], 1e-4);
    try testing.expectApproxEqAbs(expected_w, result.v[2], 1e-4);
    try testing.expectApproxEqAbs(expected_t, result.v[3], 1e-2);
}
...
test "triangleIntersect_ray_no_direction" {
    ...
    const result = ray.triangleIntersect(&a, &b, &c, true);
